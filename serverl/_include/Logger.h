...
/**
 * @brief 将 LogLevel 转换为对应的字符串表示。
 *
 * @param level 日志级别。
 * @return const char* 对应日志级别的字符串表示。
 */
...
/**
 * @sa FileLogHandler
 * @brief 文件日志处理器，输出日志到指定的日志文件。
 */
class FileLogHandler :public LogHandler {
public:
    /**
     * @brief 构造函数，初始化文件日志处理器并启动异步日志写入线程。
     *
...
public:
    ...
    // 记录日志
    void flog(LogLevel level, const char* fmt, ...);  //日志文件模式
