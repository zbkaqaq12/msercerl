...
/**
 * @brief 初始化信号系统，屏蔽不希望接收的信号并为信号设置处理函数。
 *
 * 该方法会初始化信号屏蔽字，屏蔽不希望接收的信号（如 `SIGCHLD`、`SIGINT` 等）。
 * 然后，它会为信号列表中的信号注册相应的处理函数。
 *
...
/**
 * @brief 解封指定信号并为其设置新的处理函数。
 *
 * 该方法将解封指定的信号，并设置相应的处理函数。如果信号已被屏蔽，它会被解封并为其设置新的处理函数。
 *
 * @param signo 信号编号。
...
void MSignal::unmask_and_set_handler(int signo, SignalHandlerFunction handler)
{
    ...
    sigaddset(&current_set, signo);  // 仅解封目标信号
    if (sigprocmask(SIG_UNBLOCK, &current_set, nullptr) == -1) {
        globallogger->flog(LogLevel::ALERT, "sigprocmask()解封信号失败!");
    }
 ...
 * @brief 默认的信号处理函数。
 *
 * 该函数会在接收到信号时被调用。它会输出接收到的信号编号，并显示发送信号的进程 ID（如果有）。
 *
 * @param signo 信号编号。
 * @param siginfo 信号信息。
 ...
 *
 * 该函数会处理 `SIGCHLD` 信号，回收已退出的子进程并显示子进程的退出状态。
 *
 * @param signo 信号编号。
 * @param siginfo 信号信息。
 * @param ucontext 上下文指针。
 ...
 *
 * 该方法将信号编号、信号名称和信号处理函数作为参数，添加到信号列表 `signals_` 中。
 *
 * @param signo 信号编号。
 * @param signame 信号名称。
 * @param handler 信号处理函数。
